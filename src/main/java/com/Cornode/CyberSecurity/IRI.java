...
import com.cornode.iri.network.TransactionRequester;
import com.cornode.iri.service.TipsManager;
import com.cornode.iri.storage.FileExportProvider;
...
import com.cornode.iri.network.UDPReceiver;
import com.cornode.iri.network.replicator.Replicator;
import com.cornode.iri.network.replicator.ReplicatorSinkPool;
import com.cornode.iri.network.replicator.ReplicatorSourcePool;
...
/**
 * Main cornode Reference Implementation starting class
 */
public class IRI {
    ...
    public static void main(final String[] args) throws IOException {
        ...
        cornode = new cornode(configuration);
        ixi = new IXI(cornode);
        api = new API(cornode, ixi);
        shutdownHook();
        ...
        try {
            ...
            log.error("Exception during cornode node initialisation: ", e);
            System.exit(-1);
        }
        log.info("cornode Node initialised correctly.");
    }
    ...
    private static void validateParams(final Configuration configuration, final String[] args) throws IOException {
        ...
        if (args == null || (args.length < 2 && !configurationInit)) {
            log.error("Invalid arguments list. Provide ini-file 'cornode.ini' or API port number (i.e. '-p 14600').");
            printUsage();
        }
        ...
        if (cport == null) {
            log.error("Invalid arguments list. Provide at least the PORT in cornode.ini or with -p option");
            printUsage();
        }
        else {
    ...
    private static void shutdownHook() {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            ...
            log.info("Shutting down cornode node, please hold tight...");
            try {
                ixi.shutdown();
                api.shutDown();
                cornode.shutdown();
            } catch (final Exception e) {
                log.error("Exception occurred shutting down cornode node: ", e);
            }
        }, "Shutdown Hook"));
    }
}
